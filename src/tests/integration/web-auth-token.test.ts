...
const authSource = readFileSync(
	join(projectRoot, "web", "lib", "auth.ts"),
	"utf-8",
);
...
test("auth.ts persists token to localStorage on extraction", () => {
	assert.match(
		...
		"should persist token to localStorage after extracting from hash",
	);
});
...
test("auth.ts falls back to localStorage when hash is absent", () => {
	assert.match(
		...
		"should read from localStorage when URL hash is empty",
	);
});
...
test("auth.ts defines an auth storage key constant", () => {
	assert.match(
		authSource,
		/AUTH_STORAGE_KEY/,
		"should use a named constant for the localStorage key",
	);
});
...
test("auth.ts clears the URL fragment after token extraction", () => {
	assert.match(
		authSource,
		/replaceState/,
		"should clear the hash from the address bar",
	);
});
...
test("auth.ts wraps localStorage calls in try/catch for private browsing", () => {
	...
	const beforeSetItem = authSource.slice(
		Math.max(0, setItemIndex - 200),
		setItemIndex,
	);
	const beforeGetItem = authSource.slice(
		Math.max(0, getItemIndex - 200),
		getItemIndex,
	);
	assert.match(
		beforeSetItem,
		/try\s*\{/,
		"localStorage.setItem should be inside a try block",
	);
	assert.match(
		beforeGetItem,
		/try\s*\{/,
		"localStorage.getItem should be inside a try block",
	);
});
...
const appShellSource = readFileSync(
	join(projectRoot, "web", "components", "gsd", "app-shell.tsx"),
	"utf-8",
);
...
test("app-shell.tsx sendBeacon includes auth token as query parameter", () => {
	...
	assert.match(
		appShellSource,
		/_token=/,
		"sendBeacon URL should include _token query parameter",
	);
});
...
test("proxy.ts skips auth when GSD_WEB_AUTH_TOKEN is not set", () => {
	assert.match(
		proxySource,
		/GSD_WEB_AUTH_TOKEN/,
		"proxy should read GSD_WEB_AUTH_TOKEN from env",
	);
	assert.match(
		proxySource,
		/NextResponse\.next\(\)/,
...
test("proxy.ts rejects mismatched Origin headers with a 403 origin mismatch error", () => {
	assert.match(
		proxySource,
		/request\.headers\.get\("origin"\)/,
		"proxy should inspect the Origin header",
	);
	assert.match(
		proxySource,
		/Forbidden: origin mismatch/,
		"proxy should return the origin mismatch error message",
	);
	assert.match(
		proxySource,
		/status:\s*403/,
		"proxy should reject origin mismatches with HTTP 403",
	);
});

test("proxy.ts supports extra allowed origins from GSD_WEB_ALLOWED_ORIGINS", () => {
	assert.match(
		proxySource,
		/GSD_WEB_ALLOWED_ORIGINS/,
		"proxy should read extra allowed origins from env",
	);
	assert.match(
		proxySource,
		/allowed\.has\(origin\)/,
		"proxy should compare incoming Origin against the allowed-origin set",
	);
});
