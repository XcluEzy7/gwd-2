...
export function getAllKeyStatuses(auth: AuthStorage): KeyStatus[] {
	return PROVIDER_REGISTRY.map((provider) => {
		...
		const creds = rawCreds.filter(
			(c) => !(c.type === "api_key" && !(c as ApiKeyCredential).key),
		);
...
export async function handleAddKey(
	...
	if (providerArg) {
		...
		if (!provider) {
			ctx.ui.notify(
				`Unknown provider: "${providerArg}". Use /gsd keys list to see available providers.`,
				"error",
			);
			return false;
		}
		...
		const options = PROVIDER_REGISTRY.map((p) => {
			const creds = auth
				...
				.filter((c) => !(c.type === "api_key" && !(c as ApiKeyCredential).key));
	...
	const input = await ctx.ui.input(
		...
		provider.envVar
			? `or set ${provider.envVar} env var`
			: "paste your key here",
	);
...
export async function handleRotateKey(
	...
	const creds = auth.getCredentialsForProvider(provider.id);
	const apiKeyCreds = creds.filter(
		(c) => c.type === "api_key",
	) as ApiKeyCredential[];

	if (apiKeyCreds.length === 0) {
		ctx.ui.notify(
			`No API keys for ${provider.label} (may use OAuth instead).`,
			"info",
		);
		return false;
	}
	...
	ctx.ui.notify(
		`Current key${apiKeyCreds.length > 1 ? "s" : ""}: ${currentDesc}`,
		"info",
	);
	...
	if (shouldTest) {
		...
		if (result.status === "invalid") {
			ctx.ui.notify(
				`Key validation failed: ${result.message}. Rotation cancelled.`,
				"error",
			);
			return false;
		}
		...
		if (result.status === "valid") {
			ctx.ui.notify(
				`Key validated successfully (${result.latencyMs}ms).`,
				"success",
			);
		} else {
			ctx.ui.notify(
				`Key test result: ${result.message}. Proceeding anyway.`,
				"warning",
			);
		}
	}
	...
	ctx.ui.notify(
		`Key rotated for ${provider.label}: ${maskKey(newKey)}`,
		"success",
	);
	return true;
}
...
export function runKeyDoctor(auth: AuthStorage): DoctorFinding[] {
	...
	const hasAnyLlm = llmProviders.some((p) => {
		...
		const hasValidKey = creds.some((c) =>
			c.type === "api_key" ? !!(c as ApiKeyCredential).key : true,
		);
...
export async function handleKeys(
	...
	switch (subcommand) {
		...
		case "validate": {
			...
			ctx.ui.notify(
				`Testing ${providers.length} key${providers.length > 1 ? "s" : ""}...`,
				"info",
			);
