...
import { validateProviderCredential } from "./resources/extensions/gsd/key-manager.js";
...
async function loadClack(): Promise<ClackModule> {
	try {
		...
		throw new Error(
			"[gsd] @clack/prompts not found — onboarding wizard requires this dependency",
		);
	}
}
...
export async function runOnboarding(authStorage: AuthStorage): Promise<void> {
	...
	try {
		...
		process.stderr.write(
			`[gsd] Onboarding wizard unavailable: ${err instanceof Error ? err.message : String(err)}\n`,
		);
		return;
	}
	...
	try {
		...
		p.log.warn(
			`LLM setup failed: ${err instanceof Error ? err.message : String(err)}`,
		);
		p.log.info(
			"You can configure your LLM provider later with /login inside GSD.",
		);
	}
	...
	try {
		searchConfigured = await runWebSearchStep(
			p,
			pc,
			authStorage,
			llmConfigured,
		...
		p.log.warn(
			`Web search setup failed: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
	...
	try {
		...
		p.log.warn(
			`Remote questions setup failed: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
	...
	try {
		...
		p.log.warn(
			`Tool key setup failed: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
	...
	if (llmConfigured) {
		...
		const authed = authStorage
			.list()
			.filter((id) => LLM_PROVIDER_IDS.includes(id));
		if (authed.length > 0) {
			const name = authed[0];
		...
		summaryLines.push(
			`${pc.yellow("↷")} LLM provider: skipped — use /login inside GSD`,
		);
	}
	...
	if (searchConfigured) {
		...
		summaryLines.push(
			`${pc.dim("↷")} Web search: not configured — use /search-provider inside GSD`,
		);
	}
	...
	if (remoteConfigured) {
		...
		summaryLines.push(
			`${pc.dim("↷")} Remote questions: not configured — use /gsd remote inside GSD`,
		);
	}

	if (toolKeyCount > 0) {
		summaryLines.push(
			`${pc.green("✓")} ${toolKeyCount} tool key${toolKeyCount > 1 ? "s" : ""} saved`,
		);
	} else {
...
async function runLlmStep(
	...
	if (existingAuth) {
		authOptions.push({
			value: "keep",
			label: `Keep current (${existingAuth})`,
	...
	authOptions.push(
		{
			...
			hint: "recommended — same login as claude.ai / ChatGPT",
		},
		{
			value: "api-key",
			label: "Paste an API key",
			hint: "from your provider dashboard",
		},
		{
			value: "skip",
			label: "Skip for now",
			hint: "use /login inside GSD later",
		},
	);

	const method = await p.select({
		message: existingAuth
			? `LLM provider: ${existingAuth} — change it?`
	...
	if (method === "api-key") {
		...
		const label =
			provider === "anthropic"
				...
				: provider === "openai"
					? "OpenAI"
					: (OTHER_PROVIDERS.find((op) => op.value === provider)?.label ??
						String(provider));
...
async function runOAuthFlow(
	...
	oauthMap: Map<
		string,
		{ id: string; name?: string; usesCallbackServer?: boolean }
	>,
): Promise<boolean> {
...
async function runApiKeyFlow(
	...
	if (
		expectedPrefixes &&
		!expectedPrefixes.some((pfx) => trimmed.startsWith(pfx))
	) {
		p.log.warn(
			`Key doesn't start with expected prefix (${expectedPrefixes.join(" or ")}). Saving anyway.`,
		);
	}

	if (
		providerId === "nano-gpt" ||
		providerId === "nano-gpt-payg" ||
		providerId === "ollama-cloud"
	) {
		const validation = await validateProviderCredential(providerId, trimmed);
		if (!validation.ok) {
			p.log.warn(`${providerLabel} validation failed: ${validation.message}`);
			return false;
		}
		p.log.success(
			`${pc.green(providerLabel)} validated (${validation.latencyMs ?? 0}ms)`,
		);
	}
...
async function runOllamaLocalFlow(
	...
	try {
		...
		if (response.ok) {
			...
			p.log.success(
				`${pc.green("Ollama (Local)")} configured — no API key needed`,
			);
			p.log.info(
				pc.dim(
					"Models are discovered automatically from your local Ollama instance.",
				),
			);
			return true;
		} else {
		...
		p.log.info(
			pc.dim('Install Ollama from https://ollama.com and run "ollama serve"'),
		);
	...
	const proceed = await p.confirm({
		message:
			"Save Ollama as your provider anyway? (it will auto-detect when running)",
	});
	...
	authStorage.set("ollama", { type: "api_key", key: "ollama" });
	p.log.success(
		`${pc.green("Ollama (Local)")} saved — models will appear when Ollama is running`,
	);
	return true;
}
...
async function runOllamaCloudFlow(
	...
	const method = await p.select({
		...
		options: [
			{
				value: "signin",
				label: "Sign in with Ollama",
				hint: "opens browser, uses local daemon",
			},
			{
				value: "api-key",
				label: "Paste API key",
				hint: "from ollama.com/settings/api-keys",
			},
			{
				value: "skip",
				label: "Skip for now",
			...
			const proc = execFile(
				...
				(err, stdout, stderr) => {
					...
					if (err) {
						...
						p.log.info(
							pc.dim("Make sure Ollama is installed from https://ollama.com"),
						);
						resolve(false);
						return;
					}
					// Store placeholder credential so the provider is recognized
					authStorage.set("ollama-cloud", {
						type: "api_key",
						key: "ollama-signin",
					});
					p.log.success(
						`${pc.green("Ollama Cloud")} configured via ollama signin`,
					);
					p.log.info(
						pc.dim("Cloud models are now available. Use /model to select one."),
					);
					resolve(true);
				},
			);
	...
	if (method === "api-key") {
		...
		if (!validation.ok) {
			p.log.warn(`Ollama Cloud validation failed: ${validation.message}`);
			return false;
		}
		...
		p.log.info(
			pc.dim(
				"Models are discovered automatically from ollama.com/api/v1/models",
			),
		);
		return true;
	}
...
async function runWebSearchStep(
	...
	if (existingSearch) {
		options.push({
			value: "keep",
			label: `Keep current (${existingSearch})`,
			hint: "already configured",
		});
	}
	...
	options.push(
		{
			value: "brave",
			label: "Brave Search",
			hint: "requires API key — brave.com/search/api",
		},
		{ value: "tavily", label: "Tavily", hint: "requires API key — tavily.com" },
		{
			value: "skip",
			label: "Skip for now",
			hint: "use /search-provider inside GSD later",
		},
	);
	...
	if (choice === "anthropic-native") {
		p.log.success(
			`Web search: ${pc.green("Anthropic built-in")} — works out of the box`,
		);
...
async function runToolKeysStep(
	...
	const missing = TOOL_KEYS.filter(
		(tk) => !authStorage.has(tk.provider) && !process.env[tk.envVar],
	);
...
async function runRemoteQuestionsStep(
	...
	if (existingChannel) {
		options.push({
			value: "keep",
			label: `Keep current (${existingChannel})`,
			hint: "already configured",
		});
	}
	...
	options.push(
		{
			value: "discord",
			label: "Discord",
			hint: "receive questions in a Discord channel",
		},
		{
			value: "slack",
			label: "Slack",
			hint: "receive questions in a Slack channel",
		},
		{
			value: "telegram",
			label: "Telegram",
			hint: "receive questions via Telegram bot",
		},
		{
			value: "skip",
			label: "Skip for now",
			hint: "use /gsd remote inside GSD later",
		},
	);
	...
	if (choice === "telegram") {
		...
		if (!/^\d+:[A-Za-z0-9_-]+$/.test(trimmed)) {
			p.log.warn(
				"Invalid token format — Telegram bot tokens look like 123456789:ABCdefGHI...",
			);
			return null;
		}
		...
		try {
			...
			s.stop(
				`Telegram bot: ${pc.green(data.result.first_name ?? data.result.username ?? "bot")}`,
			);
		} catch {
		...
		try {
			const res = await fetch(
				`https://api.telegram.org/bot${trimmed}/sendMessage`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						chat_id: trimmedChatId,
						text: "GSD remote questions connected.",
					}),
					signal: AbortSignal.timeout(15_000),
				},
			);
			...
			if (!data?.ok) {
				ts.stop(
					`Could not send to chat: ${data?.description ?? "unknown error"}`,
				);
				return null;
			}
...
async function runDiscordChannelStep(
	...
	try {
		...
		p.log.warn(
			"Could not fetch Discord servers — configure channel later with /gsd remote discord",
		);
		return null;
	}
	...
	if (guilds.length === 0) {
		p.log.warn(
			"Bot is not in any Discord servers — configure channel later with /gsd remote discord",
		);
		return null;
	}
	...
	try {
		...
		channels = Array.isArray(data)
			? data.filter((ch: any) => ch.type === 0 || ch.type === 5)
			: [];
	} catch {
		p.log.warn(
			"Could not fetch channels — configure later with /gsd remote discord",
		);
		return null;
	}
	...
	if (channels.length === 0) {
		p.log.warn(
			"No text channels found — configure later with /gsd remote discord",
		);
		return null;
	}
	...
	p.log.success(
		`Discord channel: ${pc.green(channelName ? `#${channelName}` : channelId)}`,
	);
